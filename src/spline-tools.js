...
export function drawSpline(ctx, pts, t, closed = false, showDetails = false) {
    ctx.lineWidth = 4;
    ctx.save();
...
export function getSplineUntyped(ctx, pts, t, closed = false, showDetails = false) {
    ctx.save()
    ...
    if (closed) {
        ...
        ctx.moveTo(pts[len - 2], pts[len - 1]);
        ctx.quadraticCurveTo(cp[2 * len - 10], cp[2 * len - 9], pts[len - 4], pts[len - 3]);
    }
    for (var i = 0; i < len; i += 2) {
        drawPoint(ctx, pts[i], pts[i + 1], 2.5, "#ffff00");
    }

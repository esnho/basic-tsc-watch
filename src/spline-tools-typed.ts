...
export function HSVtoRGB(h: number, s: number, v: number, opacity?: string) {
  // inputs h=hue=0-360, s=saturation=0-1, v=value=0-1
  // algorithm from Wikipedia on HSV conversion
  var toHex = function (decimalValue: number, places?: number) {
...
export function hexToCanvasColor(hexColor: string, opacity: string | number) {
...
export function drawPoint(ctx: CanvasRenderingContext2D, x: any, y: any, r: number, color: string) {
  ctx.save();
  ctx.beginPath();
  ctx.lineWidth = 1;
...
export function getControlPoints(x0: number, y0: number, x1: number, y1: number, x2: number, y2: number, t: number) {
...
export function drawControlLine(ctx: CanvasRenderingContext2D, x: any, y: any, px: any, py: any) {
...
export function drawSpline(ctx: CanvasRenderingContext2D, pts: any[], t: number, closed = false, showDetails = false) {
  ctx.lineWidth = 4;
  ctx.save();
  var cp: number[] = [];   // array of control points, as x0,y0,x1,y1,...
  var n = pts.length;
...
function main(t: number) {
  var e = document.getElementById("canvas1") as HTMLCanvasElement;
  e.width = 600;
  e.height = 550;
  (e.parentNode as HTMLElement).style.width = e.width + "px";  //  The div around the canvas element should fit snugly.
  const ctx = e.getContext('2d');
  if (!ctx) { return }
  ctx.clearRect(0, 0, e.width, e.height);
  ctx.scale(1.5, 1.5);
  const showDetails = (document.getElementById('details') as HTMLFormElement).checked;
  ...
  drawSpline(ctx, [20, 50, 100, 100, 150, 50, 200, 150, 250, 50, 300, 70, 310, 130, 380, 30], t, false, showDetails);
  drawSpline(ctx, [50, 200, 150, 200, 150, 300, 50, 300], t, true, showDetails);
  drawSpline(ctx, [260, 240, 360, 240, 310, 340], t, true, showDetails);
  //   Update the passive display of tension t.
  (document.getElementById("t") as HTMLFormElement).value = Math.round(1000 * t) / 1000;
}

export function getSpline(pts: any[], t: number, closed = false, showDetails = false): Path2D {
  ...
  if (closed) {
    ...
    for (var i = 2; i < len + 2; i += 2) {
      cmd = cmd + "\n" + `splinePath.moveTo(${pts[i]}, ${pts[i + 1]});`;
      splinePath.moveTo(pts[i], pts[i + 1]);
      cmd = cmd + "\n" + ``;
      splinePath.bezierCurveTo(cp[2 * i - 2], cp[2 * i - 1], cp[2 * i], cp[2 * i + 1], pts[i + 2], pts[i + 3]);
    }
    
    splinePath.moveTo(pts[0], pts[1]);
    for (var i = 2; i < len + 2; i += 2) {
      cmd = cmd + "\n" + `splinePath.moveTo(${pts[i]}, ${pts[i + 1]});`;
      splinePath.lineTo(pts[i], pts[i + 1]);
      cmd = cmd + "\n" + ``;
    }
  } else {
    // Draw an open curve, not connected at the ends
    ...
    //  For open curves the first and last arcs are simple quadratics.
    splinePath.moveTo(pts[0], pts[1]);
    splinePath.quadraticCurveTo(cp[0], cp[1], pts[2], pts[3]);

    splinePath.moveTo(pts[len - 2], pts[len - 1]);
    splinePath.quadraticCurveTo(cp[2 * len - 10], cp[2 * len - 9], pts[len - 4], pts[len - 3]);
  }
